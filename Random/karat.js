...
function stories(endings, choices,option){  
  let currentPage;
  let counter={};
  for( currentPage=1 ;!endings.includes(currentPage); currentPage++){
    for(let currentChoice=0;currentChoice<choices.length;currentChoice++){       
      if(currentPage == choices[currentChoice][0]){
        ...
        if(counter[currentPage]>2){
            return -1;
        }

        if(option =="1"){
          currentPage= choices[currentChoice][1]
        }else if(option =="2"){
          currentPage= choices[currentChoice][2]
        }
...
console.log(stories(endings4,choices4_1,1));