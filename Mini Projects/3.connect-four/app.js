document.addEventListener("DOMContentLoaded", () => {
  // all the winning combination for 4 markers in a rows or columns or diagonals
  const winningArrays = [
    [0, 1, 2, 3],
    [41, 40, 39, 38],
    [7, 8, 9, 10],
  ...
  for (let i = 0; i < squares.length; i++) {
    ...
    squares[i].onclick = () => {
      if (
        ...
        alert("start from bottom to fill upside");
      }
  ...
  function checkWinningCombination() {
    for (let index = 0; index < winningArrays.length; index++) {
      //retrieving winning combinations from winningArrays
      const s1 = squares[winningArrays[index][0]];
      const s2 = squares[winningArrays[index][1]];
      const s3 = squares[winningArrays[index][2]];
      const s4 = squares[winningArrays[index][3]];
